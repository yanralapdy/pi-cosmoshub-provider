...
import {
  COSMOSHUB_DEFAULT_MODEL_METADATA,
  COSMOSHUB_MODEL_METADATA,
} from "./models.ts";
...
async function fetchCosmosHubModels(apiKey: string): Promise<Array<{ id: string; name: string; reasoning: boolean; contextWindow: number; maxTokens: number }>> {
  try {
    ...
    return rawModels.map((m: any) => {
      ...
      const metadata = COSMOSHUB_MODEL_METADATA[id] ?? COSMOSHUB_DEFAULT_MODEL_METADATA;
      const reasoning =
        id.includes("opus") ||
        id.includes("sonnet") ||
        id.includes("pro") ||
      ...
      const { contextWindow, maxTokens } = metadata;
    ...
    return [
      { id: "qwen-3.7-max", name: "Qwen 3.7 Max", reasoning: true, ...COSMOSHUB_MODEL_METADATA["qwen-3.7-max"] },
      { id: "claude-opus-4.8", name: "Claude Opus 4.8", reasoning: true, ...COSMOSHUB_MODEL_METADATA["claude-opus-4.8"] },
      { id: "deepseek-v4-pro", name: "DeepSeek V4 Pro", reasoning: true, ...COSMOSHUB_MODEL_METADATA["deepseek-v4-pro"] },
    ];
  }
}
